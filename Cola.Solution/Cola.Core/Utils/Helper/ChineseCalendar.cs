...
namespace Cola.Core.Utils.Helper;
...
public class ChineseCalendar
{
    ...
    private static readonly DateTime MinDay = new(1900, 1, 30);
    ...
    public ChineseCalendar(DateTime dt)
    {
        ...
        leap = GetChineseLeapMonth(ChineseYear); //计算该年闰哪个月
        //设定当年是否有闰月
        if (leap > 0)
            IsChineseLeapYear = true;
        else
            IsChineseLeapYear = false;
    ...
    public ChineseCalendar(int cy, int cm, int cd, bool leapMonthFlag)
    {
        ...
        var leap = GetChineseLeapMonth(cy); // 计算该年应该闰哪个月
        if (leap != 0)
            IsChineseLeapYear = true;
        else
            IsChineseLeapYear = false;
        ...
        if (cm != leap)
            IsChineseLeapMonth = false; //当前日期并非闰月
        else
            IsChineseLeapMonth = leapMonthFlag; //使用用户输入的是否闰月月份


        if (IsChineseLeapYear == false || //当年没有闰月
            cm < leap) //计算月份小于闰月     
        {
            #region ...
    ...
    private bool BitTest32(int num, int bitpostion)
    {
        if (bitpostion > 31 || bitpostion < 0)
            throw new ColaException("Error Param: bitpostion[0-31]:" + bitpostion);
        ...
        get
        {
            ...
            if (IsChineseLeapMonth == false) //闰月不计算节日
            {
                foreach (var lh in LHolidayInfo)
                    if (lh.Month == ChineseMonth && lh.Day == ChineseDay)
                    {
    ...
    public string GanZhiYearString
    {
        ...
        {
            string tempStr;
            var i = (ChineseYear - GanZhiStartYear) % 60; //计算干支
    ...
    public string GanZhiMonthString
    {
        get
        {
            ...
            gan = GanStr[(ganIndex + ChineseMonth - 2) % 10].ToString();
