using System.Text;
using Cola.Core.Models.Core.CaSnowFlake;
using Cola.Core.Utils.Enums;

namespace Cola.Core.SnowFlake;
...
public class ColaSnowFlake : IColaSnowFlake
{
    ...
    public ColaSnowFlake(SnowFlakeModel model)
    {
        ...
        if (model.DatacenterId > MaxDatacenterId || model.DatacenterId < 0)
            throw new Utils.ColaException(EnumException.SnowFlakeDatacenterIdLengthGtOrLt, MaxDatacenterId.ToString());
        if (model.WorkerId > MaxWorkerId || model.WorkerId < 0)
            throw new Utils.ColaException(EnumException.SnowFlakeWorkerIdLengthGtOrLt, MaxWorkerId.ToString());
